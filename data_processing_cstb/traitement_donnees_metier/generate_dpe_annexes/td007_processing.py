...
def agg_surf_envelope(td007, td008):
    ...
    surf_vitree_orient = td008_vitree.pivot_table(index='td001_dpe_id', columns='orientation_baie_infer',
...
def agg_td007_to_td001_essential(td007):
    ...
    mat_ph_agg = agg_pond_top_freq(td007_ph, 'mat', 'surf_paroi_opaque_deperditive_infer',
...
def generate_mur_table(td007):
    ...
    td007_mur['u'] = td007_mur['coefficient_transmission_thermique_paroi'].astype(float)
    td007_mur['u0'] = td007_mur['coefficient_transmission_thermique_paroi_non_isolee'].astype(float)
    td007_mur.tv004_umur = td007_mur.tv004_umur.astype(float)
    td007_mur.tv003_umur = td007_mur.tv003_umur.astype(float)

    # anomaly u0 vs tvu0 -> pointe vraisemblablement vers des anomalies
    not_null = (~td007_mur.u0.isnull()) & (~td007_mur.tv004_umur.isnull())
    not_null = not_null & td007_mur.u0 > 0.05
    sel = td007_mur.loc[not_null]
    bad = sel.loc[~np.isclose(sel.u0, sel.tv004_umur, atol=0.1)].index
    td007_mur['anomaly_u0_tvu0'] = 0
    td007_mur.loc[bad, 'anomaly_u0_tvu0'] = 1
    td007_mur['u0'] = td007_mur[['u0', 'tv004_umur']].max(axis=1)

    # anomaly u vs tvu -> trop d'anomalies -> cela doit être un probleme d'implémentation EDL.
    not_null = (~td007_mur.u.isnull()) & (~td007_mur.tv003_umur.isnull())
    not_null = not_null & td007_mur.u > 0.05
    sel = td007_mur.loc[not_null]
    goods = np.isclose(sel.u, sel.tv003_umur, atol=0.1)
    goods = goods | np.isclose(sel.u, sel.u0, atol=0.1)
    bad = sel.loc[~goods].index
    td007_mur['anomaly_u_tvu'] = 0
    ...
    td007_mur.loc[is_u_bad, 'u'] = td007_mur.loc[is_u_bad, 'tv003_umur']
    ...
    td007_mur['annee_construction'] = td007_mur.annee_construction.astype(float)
    td007_mur['annee_isole_uniforme_min'] = td007_mur.annee_isole_uniforme_min.astype(float)

    # si année isolation > année construction +20
    is_renove = td007_mur.annee_isole_uniforme_min > td007_mur.annee_construction + 20
    # et si année isolation >2000
    is_renove = is_renove & (td007_mur.annee_isole_uniforme_min > 2000) & (td007_mur.u <= 0.5)
    # si pas d'info sur année isolation.
    is_null = td007_mur.annee_isole_uniforme_min.isnull()
    # on regarde si performance ~=  performance bâtiments récent avec année construction ancienne.
    is_renove_null = is_null & (td007_mur.annee_construction < 1988) & (td007_mur.u.between(0.05, 0.47))
    is_renove_null = is_renove_null | (is_null & (td007_mur.annee_construction < 2000) & (td007_mur.u.between(0.05, 0.25)))
...
def agg_td007_mur_to_td001(td007_mur):
    ...
    for var in ['meth_calc_u', 'u', 'epaisseur_isolation', 'resistance_thermique_isolation', 'meth_calc_isolation',
                'annee_isole_uniforme_min', 'annee_isole_uniforme_max', 'mat', 'ep_mat',
                ]:
